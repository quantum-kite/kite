...
KITE is written in C++ with code optimisation at the design level, including automated lattice-domain decomposition to boost multithreading performance. KITE's interface & post-processing tools are based on [Pybinding][1], a scientific Python package for TB calculations.
...
First download the source code. You can find the latest version of KITE at our official repository on GitHub **[repository][3]**. Fetch the source code from the Git Hub [repository][3] using the command
``` bash
...
```
Alternatively, you might prefer to follow the instructions on the [Pybinding][1] webpage.
...
make
```
If these commands have run successfully, you will now find KITEx in the build directory and KITE-tools in the tools/build directory, which are now ready to use!
...
``` bash
../tools/build/KITE-tools example_initial.h5
```
