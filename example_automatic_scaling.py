...
disorder_struc = StructuralDisorder(lattice, concentration=0.05)
disorder_struc.add_vacancy('A')

# node0 = [[+0, +0], 'A']
# node1 = [[+0, +0], 'B']
# disorder_struc = StructuralDisorder(lattice, concentration=0.1)
...
export_lattice(lattice, configuration, calculation, modification, 'auto_scaling.h5', disorder=disorder,
               disorded_structural=disorder_struc)

# it's possible to make roughly an equivalent model using pybindig (more disorder realisations you select, more similar
# it will be)
# calculate DOS to confirm
model = make_pybinding_model(lattice, disorder=disorder, disorder_structural=disorder_struc)
