...
class shg{
    ...
    bool isRequired = false; // was this quantity (shg) asked for?
    ...
    shg(system_info<T, DIM>&, shell_input &);
    ...
    Eigen::Matrix<std::complex<T>, -1, -1> Gamma3Contract_AA();
    Eigen::Matrix<std::complex<T>, -1, -1> Gamma3Contract_RR();
...
template <typename T, unsigned DIM>
shg<T, DIM>::shg(system_info<T, DIM>& info, shell_input & vari){
  // Constructor of the shg class. This function simply checks
  ...
  // check whether the shg was asked for
  try{
...
template <typename T, unsigned DIM>
void shg<T, DIM>::override_parameters(){
...
template <typename T, unsigned DIM>
void shg<T, DIM>::fetch_parameters(){
	debug_message("Entered shg::read.\n");
  ...
  if(special == 1){
	...
	file.close();
	debug_message("Left shg::read.\n");
}

template <typename U, unsigned DIM>
void shg<U, DIM>::calculate(){
